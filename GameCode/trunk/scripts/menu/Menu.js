...
class Menu {
  ...
  constructor(player) {
    ...
    this.controlsArrowKeys.src = "./images/Arrow-buttons.png";
    this.controlVKey = new Image();
    this.controlVKey.src = "./images/vKey.png";
  ...
  draw(scoreboard) {
    ...
    switch (this.menuState) {
      case MENU_OPTIONS.PLAY:
        ...
        this.retry = false; // reset the retry flag so that the User can choose Play Again when the Game ends
        break;
  ...
  drawHowToPlayMenu() {
    ...
    lines.forEach((line, index) => {
      CANVAS.CONTEXT.fillText(
        ...
        INSTRUCTIONS.text.yPos + INSTRUCTIONS.FONT_SIZE * index * 2
      );
    });
  }
  ...
  drawControlsMenu() {
    ...
    CANVAS.CONTEXT.drawImage(this.controlsArrowKeys, 90, 200, 200, 120);
    CANVAS.CONTEXT.drawImage(this.controlVKey, 450, 250, 50, 60);
  ...
  keydownHandler(event) {
    ...
    if (event.key === "Enter") {
      // if User is on the Game Over screen and chooses to Play Again
      if (this.selection === MENU_OPTIONS.GAME_OVER) {
        this.selection = MENU_OPTIONS.PLAY; // reset the cursor to be drawn around 'Play Game' option
        this.menuState = "Main Menu"; // display the Main Menu
        this.retry = true; // Indicate User has selected to Play Again
        this.playGame = false; // User will return to the Main Menu and not the Running Game
      } else {
        this.menuState = this.selection;
      }
    }
   ...
   * @function
   *    - will be called continuously in the script startGame method at the Menu section
   ...
   * @file Menu.js
   * @method toPlayAgain()
   * @description determine if the User has selected to Play the Game again
   * @function
   *    - will be called continuously in the startGame method at the Game Over section
   * @param none
   * @returns {boolean} retry - true if User selected "Play Again" Option, false otherwise
   */
  toPlayAgain() {
    return this.retry;
  }
