...
class EnemyGridController {
   ...
   */
  createEnemies() {
    this.enemyGrid.length = 0; // clear the current grid of Enemies
    for (let i = 1; i <= GRID_SIZE.ROWS; i++) {
      ...
      for (let j = 1; j <= GRID_SIZE.COLUMNS; j++) {
        ...
        const enemy = new Enemy(xOffset, yOffset);
  ...
  shoot(enemyProjectileController) {
    ...
    enemyProjectileController.draw(); // shoot all the Enemy projectiles
  }
