...
class Player {
  ...
  drawPlayer() {
    ...
    this.movePlayer();
    this.shoot();
    CANVAS.CONTEXT.drawImage(this.sprite, this.xPosition, this.yPosition, this.width, this.height);
  }
  ...
  shoot() {
    ...
    this.BulletControl.draw(); // shoots the projectile
  }

  /**
   * @todo IMPLEMENT THE FUNCTION TO ANIMATE THE PLAYER SPRITE
   *
   * @file Player.js
   * @method updatePlayerImage()
   * @description change the image of the Player sprite
   * @function
   *    - update the image to handle the different Player states:
   *        - Idle
   *        - Shooting
