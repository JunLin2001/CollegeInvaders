...
class Enemy {
  constructor(canvasWidth, canvasHeight, xOffset, yOffset, laserController) {
    ...
    // controller object to manage when and how enemy shoots lasers
  ...
  // shoot  // enemy ability to shoot lasers at the player
  shoot(canvasCtx) {
    const speed = -1; // value must be negative for bullet to travel downwards towards the player
    const delay = Math.floor(Math.random() * 500) + 200; // time before next shot
    const hit = true;
    const bullet_x = this.xPosition + this.width / 2;
    const bullet_y = this.yPosition + this.height;
    this.laserController.shoot(bullet_x, bullet_y, speed, hit, delay);
    this.laserController.draw(canvasCtx);
  }
